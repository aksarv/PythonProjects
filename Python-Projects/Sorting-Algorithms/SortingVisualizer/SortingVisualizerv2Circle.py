...
def circle(lst, l, h):
    circle_frames.append([lst.copy(), l, h, lst[l], lst[h]])
    swapped = False
    for i in range(l, (l+h)//2+1):
        if lst[i] > lst[h-(i-l)]:
            lst[i], lst[h-(i-l)] = lst[h-(i-l)], lst[i]
            swapped = True
    return lst, swapped

def get_frames_circle_sort(lst, low, high):
    if low >= high:
        return lst
    lst, swapped = circle(lst, low, high)
    lst = get_frames_circle_sort(lst, low, (low+high)//2)
    lst = get_frames_circle_sort(lst, (low+high)//2+1, high)

    if swapped:
        get_frames_circle_sort(lst, low, high)
...
get_frames_circle_sort(test_lst, 0, len(test_lst)-1)
...
while run:
    ...
    try:
        curr_radix_frame = circle_frames[curr_frame][0]
    except IndexError:
        pygame.quit()
        quit()
    ...
    fp = circle_frames[curr_frame][1]
    rp = circle_frames[curr_frame][2]
    v1 = circle_frames[curr_frame][3]
    v2 = circle_frames[curr_frame][4]
