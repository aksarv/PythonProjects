...
environment = [np.array([-1, 4, -1, 1, 0.8, 60, 0, 0]), np.array([0, 5, 0, 1, 0.8, 0, 0, 60])]
camera = np.array([0, 0, 0])
light_sources = [np.array([0, 0, 1, 60])]
ambient_light = 1
...
for i, x in enumerate(np.linspace(tl[2], bl[2], resolution_y)):
    for j, y in enumerate(np.linspace(tl[0], tr[0], resolution_x)):
        ...
        for obj in environment:
            o_x, o_y, o_z, sphere_radius, smoothness, red, green, blue = obj
