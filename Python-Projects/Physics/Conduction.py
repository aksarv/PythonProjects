...
for _ in range(num_particles):
    ...
    particles.append(
        Particle(
            ...
            random.randint(0, 1024),
            math.cos(math.radians(rand_angle)) * 100,
            math.sin(math.radians(rand_angle)) * 100,
            5,
        )
    )
        ...
        for p2 in particles:
            if p1.id != p2.id:
                ...
                if math.sqrt((delta_y) ** 2 + (delta_x) ** 2) < p1.width + p2.width:
                    difference = (p1.width + p2.width) - math.sqrt(
                        (delta_y) ** 2 + (delta_x) ** 2
                    )
                    ...
                    p2.x += delta_x_1
                    new_p1_x_vel = (
                        (p1.width - p2.width) * p1.x_vel + 2 * p2.width * p2.x_vel
                    ) / (p1.width + p2.width)
                    new_p1_y_vel = (
                        (p1.width - p2.width) * p1.y_vel + 2 * p2.width * p2.y_vel
                    ) / (p1.width + p2.width)
                    new_p2_x_vel = (
                        (p2.width - p1.width) * p2.x_vel + 2 * p1.width * p1.x_vel
                    ) / (p1.width + p2.width)
                    new_p2_y_vel = (
                        (p2.width - p1.width) * p2.y_vel + 2 * p1.width * p1.y_vel
                    ) / (p1.width + p2.width)
    ...
    for p in particles:
        pygame.draw.circle(
            ...
            (
                min(
                    (
                        math.sqrt(p.x_vel**2 + p.y_vel**2) - 100
                        if math.sqrt(p.x_vel**2 + p.y_vel**2) - 100 > 0
                        else 0
                    )
                    / 300
                    * 255,
                ...
                0,
                255 - min(math.sqrt(p.x_vel**2 + p.y_vel**2) / 300 * 255, 255),
            ),
            (p.x, p.y),
            p.width,
        )
    scr.blit(
        pygame.font.SysFont(None, 32).render(
            "Average Temperature: "
            + str(
                round(
                    (
                        sum(math.sqrt(p.x_vel**2 + p.y_vel**2) for p in particles)
                        / len(particles)
                    )
                    / 100
                    * 20,
